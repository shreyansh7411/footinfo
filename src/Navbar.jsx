...
const Navbar = () => {
    ...
    <div className='min-w-fit bg-zinc-900 flex p-1 justify-between gap-3'>
      ...
      <div className='flex justify-center items-center pr-5 relative group'>
        <a href='/contact' className="text-white text-2xl cursor-pointer transition-all duration-1000 ease-in-out transform hover:scale-110"><IoPersonSharp/></a>
      </div>
    </div>
    <div className='min-w-full flex justify-between bg-zinc-900 gap-3'>
      <div className='flex justify-evenly text-white w-1/2'>
        <button className={`top5-btn ${activea ? 'bg-gray-400  border-none rounded-t-sm text-black':''}`} onClick={() => {handleLogoClick(2021); setActivea(true); setActiveb(false); setActivec(false); setActived(false); setActivee(false);}}>
        ...
        <button className={`top5-btn ${activeb ? 'bg-gray-400  border-none rounded-t-sm text-black':''}`} onClick={() => {handleLogoClick(2014); setActivea(false); setActiveb(true); setActivec(false); setActived(false); setActivee(false);}}>
          <div className={`top5 flex justify-center pt-2 `}>
            <img src={laligaLogo} className='w-6.5 h-6.5'/>
          </div>
          Laliga
        </button>
        <button className={`top5-btn  ${activec ? 'bg-gray-400  border-none rounded-t-sm text-black':''}`} onClick={() => {handleLogoClick(2002); setActivea(false); setActiveb(false); setActivec(true); setActived(false); setActivee(false);}}>
          <div className={`top5 flex justify-center pt-2`}>
        ...
        <button className={`top5-btn  ${actived ? 'bg-gray-400  border-none rounded-t-sm text-black':''}`} onClick={() => {handleLogoClick(2019); setActivea(false); setActiveb(false); setActivec(false); setActived(true); setActivee(false);}}>
          <div className={`top5 flex justify-center pt-2`}>
      ...
      <g transform="matrix(.44962 0 0 -.44962 -1596.905 937.635)">
        ...
        <button className={`top5-btn ${activee ? 'bg-gray-400  border-none rounded-t-sm text-black':''}`} onClick={() => {handleLogoClick(2015); setActivea(false); setActiveb(false); setActivec(false); setActived(false); setActivee(true);}}>
          <div className={`top5 flex justify-center pt-2 `}>
